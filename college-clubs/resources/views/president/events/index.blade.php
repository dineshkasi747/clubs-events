...
@section('title', 'Manage Events - President Dashboard')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('president.events.create') }}" class="rounded-xl bg-gradient-to-r from-brand-600 to-indigo-500 hover:from-brand-500 hover:to-indigo-400 px-4 py-2.5 text-sm font-semibold text-white shadow-lg transition-all duration-200">
                + Onboard New Event
            </a>
        </div>
    </div>
    ...
    <div class="glass rounded-2xl p-6 border border-slate-800 bg-slate-900/30">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <form action="{{ route('president.events.upload-pdf') }}" method="POST" enctype="multipart/form-data" class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                @csrf
            ...
            @forelse($events as $evt)
                <div class="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden hover:border-slate-700/60 transition-all duration-200 flex flex-col justify-between">
                    ...
                    <div class="h-32 bg-slate-800 flex items-center justify-center relative select-none">
                        @if($evt->images->isNotEmpty())
                            <img src="{{ $evt->images->first()->path }}" class="h-full w-full object-cover" alt="{{ $evt->title }}">
                        @else
                    ...
                    <div class="p-5 space-y-4 flex-grow flex flex-col justify-between">
                        <div class="space-y-2">
                            <h4 class="font-bold text-white text-lg leading-tight line-clamp-1">{{ $evt->title }}</h4>
                            <p class="text-xs text-slate-400 line-clamp-2">{{ $evt->description ?? 'No event description entered.' }}</p>
                        </div>
                        ...
                        <div class="space-y-3 pt-3 border-t border-slate-800/80">
                            ...
                            <div class="flex justify-between items-center text-xs">
                                <div>
                                    ...
                                    <span class="font-bold text-white">{{ $evt->price > 0 ? '$' . number_format($evt->price, 2) : 'Free' }}</span>
                                </div>
                                <div class="text-right">
                                    <span class="block text-[9px] uppercase tracking-wider text-slate-500 font-semibold">REGISTRATIONS</span>
                                    <span class="font-bold text-slate-300">{{ $evt->registrations()->where('status', '!=', 'cancelled')->count() }} / {{ $evt->capacity }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-4 border-t border-slate-800/80 flex items-center justify-between gap-3">
                            <a href="{{ route('president.events.edit', $evt) }}"
                            ...
                            <form action="{{ route('president.events.destroy', $evt) }}" method="POST" class="flex-shrink-0" onsubmit="return confirm('Delete event permanently?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
