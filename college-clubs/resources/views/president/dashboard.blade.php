...
@section('title', 'President Club Hub - College Clubs & Events')
    ...
    @else
        ...
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
                    {{ $club->name }} <span class="text-indigo-400">Dashboard</span>
                </h1>
                <p class="text-slate-400 text-sm mt-1">Welcome back, President. Monitor events, registrations, and club revenue.</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('president.events.create') }}" class="rounded-xl bg-gradient-to-r from-brand-600 to-indigo-500 hover:from-brand-500 hover:to-indigo-400 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/10 transition-all duration-200">
                    + Onboard New Event
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="glass rounded-2xl p-6 relative overflow-hidden">
                ...
                <span class="block text-3xl font-extrabold text-white mt-2">{{ $stats['total_events'] }}</span>
            </div>
            ...
            <div class="glass rounded-2xl p-6 relative overflow-hidden">
                ...
                <span class="block text-3xl font-extrabold text-indigo-400 mt-2">{{ $stats['active_events'] }}</span>
            </div>
            ...
            <div class="glass rounded-2xl p-6 relative overflow-hidden">
                ...
                <span class="block text-3xl font-extrabold text-white mt-2">{{ $stats['total_registrations'] }}</span>
            </div>
            ...
            <div class="glass rounded-2xl p-6 relative overflow-hidden">
                ...
                <span class="block text-3xl font-extrabold text-emerald-400 mt-2">${{ number_format($stats['total_revenue'], 2) }}</span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <div class="glass rounded-2xl p-6 space-y-6">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('president.events.index') }}" class="text-xs font-semibold text-indigo-400 hover:text-indigo-300">View All Events &rarr;</a>
                </div>
                ...
                <div class="space-y-4">
                    @forelse($upcomingEvents as $evt)
                        <div class="flex items-center justify-between p-4 bg-slate-900/50 rounded-xl border border-slate-800/80">
                            <div>
                                <span class="block font-semibold text-white text-sm">{{ $evt->title }}</span>
                                <span class="block text-xs text-slate-400 mt-1">
                                    {{ $evt->formatted_date }} &bull; {{ $evt->venue }}
                                </span>
                            </div>
                            <div class="text-right">
                                <span class="block text-xs font-bold text-indigo-400">
                                    {{ $evt->price > 0 ? '$' . number_format($evt->price, 2) : 'FREE' }}
                                </span>
                        ...
                        @forelse($recentRegistrations as $reg)
                            <li class="py-4">
                                <div class="flex items-center justify-between text-sm">
                                    <div>
                                        <span class="block font-semibold text-white">{{ $reg->user->name }}</span>
                                        <span class="block text-xs text-slate-400">Registered for <span class="text-indigo-400">{{ $reg->event->title }}</span></span>
                                    </div>
                                    <div class="text-right">
                                        <span class="block text-xs text-slate-500">{{ $reg->created_at->diffForHumans() }}</span>
                                        @if($reg->payment && $reg->payment->status === 'completed')
                                            <span class="inline-block text-[10px] text-emerald-400 font-semibold bg-emerald-500/10 px-2 py-0.5 rounded-full mt-0.5">
                                                Paid ${{ number_format($reg->payment->amount, 2) }}
                                            </span>
                                        @else
