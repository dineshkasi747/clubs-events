...
@section('title', 'Admin Control Panel - College Clubs & Events')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.clubs.index') }}" class="rounded-xl bg-gradient-to-r from-brand-600 to-indigo-500 hover:from-brand-500 hover:to-indigo-400 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-500/10 transition-all duration-200">
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div class="glass rounded-2xl p-6 relative overflow-hidden">
            ...
            <span class="block text-3xl font-extrabold text-white mt-2">{{ $stats['total_clubs'] }}</span>
        </div>
        ...
        <div class="glass rounded-2xl p-6 relative overflow-hidden">
            ...
            <span class="block text-3xl font-extrabold text-white mt-2">{{ $stats['total_presidents'] }}</span>
        </div>
                            ...
                            <tr>
                                <td class="py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <span class="block font-semibold text-white">{{ $club->name }}</span>
                                            <span class="block text-xs text-slate-400 truncate max-w-xs">{{ $club->description }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-4">
                                    @if($club->president)
                                        <span class="block font-medium text-slate-200">{{ $club->president->name }}</span>
                                        <span class="block text-xs text-slate-500">{{ $club->president->email }}</span>
                                    @else
                                ...
                                <td class="py-4 text-right">
                                    <form action="{{ route('admin.clubs.destroy', $club) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this club? All linked events will be removed.')">
                                        @csrf
                                        @method('DELETE')
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="truncate text-xs text-slate-400">
                                        Registered for <span class="text-indigo-300">{{ $reg->event->title }}</span>
                                    </p>
